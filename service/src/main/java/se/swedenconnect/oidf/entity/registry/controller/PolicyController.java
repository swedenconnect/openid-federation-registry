...
public class PolicyController {
   ...
   *
   * @param policy a {@link PolicyDto} object containing the details of the policy to be created
   * @param response the {@link HttpServletResponse} object used to set the response status
   *
   * @return a {@link PolicyDto} object representing the created policy
   */
  @PostMapping
  public PolicyDto createPolicy(@RequestBody final PolicyDto policy, final HttpServletResponse response) {
    log.debug("POST: {}", policy);

    PolicyDto dto = this.policyService.create(policy);
   ...
   *
   * @return a list of {@link PolicyDto} representing the policy records
   */
  @GetMapping
  public List<PolicyDto> getAllPolicies() {
    List<PolicyDto> policies = this.policyService.getAll();
   ...
   *
   * @return a {@link PolicyDto} object representing the policy, if found
   */
  @GetMapping("/{name}")
  public PolicyDto getPolicyByName(@PathVariable("name") final String name) {
    log.debug("GET by name: {}", name);

    PolicyDto dto = this.policyService.get(name);
    if (dto == null) {
   ...
   * @param policy A {@link PolicyDto} object containing the updated details of the policy.
   * @return A {@link PolicyDto} object representing the updated policy.
   */
  @PutMapping("/{name}")
   public PolicyDto updatePolicy(@PathVariable("name") final String name, @RequestBody PolicyDto policy) {
