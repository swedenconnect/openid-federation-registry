...
import se.swedenconnect.oidf.entity.registry.config.SecurityConfig;
...
public class RegistryJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
  ...
  public AbstractAuthenticationToken convert(final Jwt jwt) {
    ...
    final List<OrganizationEntity> orgs = information.organizations().stream().map(orgInfo -> {
      return this.organizationService.findCreate(orgInfo.orgNumber(), orgInfo.orgName());
    }).toList();
