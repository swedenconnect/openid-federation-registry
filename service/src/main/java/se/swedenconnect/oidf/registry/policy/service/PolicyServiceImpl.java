...
public class PolicyServiceImpl implements PolicyService {
  ...
  private Policy findPolicyOrThrow(final OrganizationRecord organizationRecord, final UUID id) {
  ...
  private static PolicyDto toDto(final Policy entity) {
    return PolicyToDtoMapper.toDto(entity);
  }
  ...
  public PolicyDto updatePolicy(final OrganizationRecord organizationRecord,
      final UUID id, final PolicyDto input) {
    ValidateDto.init(organizationRecord).validate(input);
    final Policy existing = this.findPolicyOrThrow(organizationRecord, id);
    final PolicyDto oldDto = toDto(existing);

    DtoToPolicyMapper.updateEntity(existing, input);
