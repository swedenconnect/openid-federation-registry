...
public class PolicyControllerIT {
  ...
  public void testCreateMultiplePoliciesWithSameName() {
    // Arrange
    final PolicyDto policy = new PolicyDto("policy-name", "{}");

    // Act
    ResponseEntity<PolicyDto> response = this.restTemplate.postForEntity("/registry/v1/policies", policy, PolicyDto.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);

    response = this.restTemplate.postForEntity("/registry/v1/entities", policy, PolicyDto.class);
  ...
  public void testGetAllPolicies() {
    ...
    IntStream.range(13, 43).boxed().forEach(i -> {
          final PolicyDto policy = new PolicyDto("policy-name-" + i, "{}");
          this.restTemplate.postForEntity("/registry/v1/policies", policy, PolicyDto.class);
        }
    );

    // Act
    final ResponseEntity<PolicyDto[]> response = this.restTemplate.getForEntity("/registry/v1/policies", PolicyDto[].class);

    // Assert
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    final PolicyDto[] policies = response.getBody();
  ...
  public void testUpdatePolicy() {
    ...
    // Arrange
    final PolicyDto policy = new PolicyDto("openid_relying_party", policyBody);
    final ResponseEntity<PolicyDto> createResponse = this.restTemplate.postForEntity("/registry/v1/policies", policy, PolicyDto.class);
    final PolicyDto createdPolicy = createResponse.getBody();
    ...
    final HttpEntity<PolicyDto> requestUpdate = new HttpEntity<>(new PolicyDto("openid_relying_party", updatedPolicyBody));
    this.restTemplate.put("/registry/v1/policies/{name}", requestUpdate, "openid_relying_party");

    final ResponseEntity<PolicyDto> updateResponse = this.restTemplate.getForEntity("/registry/v1/policies/{name}", PolicyDto.class, "openid_relying_party");

    // Assert
    assertThat(updateResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
    final PolicyDto updatedPolicy = updateResponse.getBody();
    assertThat(updatedPolicy).isNotNull();
    assertThat(updatedPolicy.policy()).containsSequence("\\\"scope\\\" : {\\n      \\\"subset_of\\\" : [ \\\"openid\\\", \\\"profile\\\", \\\"email\\\"]");
  }
  ...
  public void testDeletePolicy() {
    ...
    final PolicyDto policy = new PolicyDto(policyName, "{}");
    ResponseEntity<PolicyDto> policyDtoResponseEntity =
        this.restTemplate.postForEntity("/registry/v1/policies", policy, PolicyDto.class);
    ...
    // Verify it's deleted
    final ResponseEntity<PolicyDto> getResponse = this.restTemplate.getForEntity("/registry/v1/policies/{name}", PolicyDto.class, policyName);
